...
public abstract class Packet {
    ...
    public static ArrayList<byte[]> read_full_result_set(InputStream in, MessageEvent messageEvent, ArrayList<byte[]> buffer, boolean bufferResultSet) throws IOException {
        // Assume we have the start of a result set already
        ...
        for (int i = 0; i < (colCount+1); i++) {
            ...
            // Evil optimization
            if (!bufferResultSet) {
                Packet.write(messageEvent, buffer);
                //out.flush();
                buffer.clear();
            }
        }
        ...
        if (EOF.loadFromPacket(packet).hasStatusFlag(Flags.SERVER_MORE_RESULTS_EXISTS)) {
            buffer.add(Packet.read_packet(in));
            buffer = Packet.read_full_result_set(in, messageEvent, buffer, bufferResultSet);
        }
        return buffer;
    }
